...
def test_filter_status_param(selector, status_log, capsys, expect_subset):
    ...
    found_codes = set(tok for ln in lines for tok in ln.split() if tok.isdigit() and len(tok) == 3)
...
def test_filter_regex_edges(regex_edge_log, capsys, pattern, expected_count, must_contain):
...
def test_filter_bytes_dash_and_rt(sample_log, capsys):
    ...
    assert line.endswith(" - rt=0.12") or line.endswith(" - rt=0.120") or " - rt=0.12" in line


def test_filter_out_file(sample_log, tmp_path, capsys):
    out_file = tmp_path / "filtered.txt"
    assert main(["filter", "--path", str(sample_log), "--status", "5xx", "--out", str(out_file)]) == 0
    # stdout пустой
