...
Тесты к этому заданию занимают время и кушают наши деньги, в задаче есть только публичные тесты, поэтому пожалуйста прогоните их локально с помощью команды `pytest`. Когда локально все будет ок, запушите.