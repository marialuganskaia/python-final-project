...
Кодовое слово, необходимое при регистрации, смотрите в канале группы
...
Вся работа по получению, решению и сдаче заданий происходит через git репозиторий. Для начала нужно его получить, скачать и настроить доступ. Подробнее про гит при необходимости можно глянуть тут [лекция про гит](https://yadi.sk/i/YUe3SJYo11EChA).
...
$ git remote add upstream git@gitlab.manytask.org:hse-advanced-python/public-2025-fall.git
```
...
- Проверьте что всё активировалось и ссылки `python` и `pip` ведут на папку с `hse_env`
  ```bash
  (hse_env)$ which python
  <директория с задачками>/hse_env/bin/python
  (hse_env)$ which pip
  <директория с задачками>/hse_env/bin/pip
  ```
...
- Поставьте пакеты:
  ...
  (hse_env)$ brew install openblas gfortran
  (hse_env)$ export OPENBLAS="$(brew --prefix openblas)"
  # Отдельно ставим биндинговые пакеты
  (hse_env)$ pip install cython pybind11 pythran
  # Ставим llvm, который нужен некоторым отдельным пакетам 
  (hse_env)$ brew install llvm@11
  (hse_env)$ export LLVM_CONFIG="/opt/homebrew/Cellar/llvm@11/11.1.0_2/bin/llvm-config"
  
  # Ставим отдельно llvmlite
  (hse_env)$ pip install llvmlite
  # Самое весёлое - пробуем собрать себе капризные библиотеки (это может занять время)
  (hse_env)$ pip install --no-binary :all: --no-use-pep517 [package]==[version]
  ```
  ...
  (hse_env)$ pip install --upgrade -r requirements.txt
  # установка локальной testlib для тестирования некоторых задачек
  (hse_env)$ pip install --upgrade --editable tools/testlib
  ```

- Проверьте версии:
  ```bash
  (hse_env)$ python --version
  Python 3.12.5
  (hse_env)$ pytest --version
  pytest 8.3.2
  (hse_env)$ flake8 --version
  7.1.1 <...>
  (hse_env)$ mypy --version
  mypy 1.11.2
  ```
  Иногда venv может криво подцеплять консольные приложения библиотек. Для уверенности используйте вызов вида 
  ```shell
  (hse_env)$ python -m pytest --version
  ```
...
- Укажите путь до установленного интерпретатора: `<директория с задачками>/hse_env/bin/python`
...
```bash
$ source hse_env/bin/activate   # активируем виртуальное окружение, если не активировано
(hse_env)$ pytest 01.1.PythonTools/tasks/hello_world/  # запуск тестов
# ...
(hse_env)$ flake8 01.1.PythonTools/tasks/hello_world/  # запуск линтера и stylecheck'а
# ...
(hse_env)$ mypy 01.1.PythonTools/tasks/hello_world/    # запуск typecheck'а
```
...
```bash
# Устанавливаем RISE
(hse_env)$ pip install rise>=5.7.0 jupyterlab-rise>=0.40.0
...
(hse_env)$ nano ~/.ipython/profile_default/startup/typecheck.py
