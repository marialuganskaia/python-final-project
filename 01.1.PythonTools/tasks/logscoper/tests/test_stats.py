...
def test_stats_json(sample_log, capsys):
    ...
    assert data["total"] == 5
    assert data["status"]["200"] == 3 and data["status"]["302"] == 1 and data["status"]["500"] == 1
    ...
    assert isinstance(data["top_paths"][0][0], str) and isinstance(data["top_paths"][0][1], int)
...
def test_stats_empty_after_filters(sample_log, capsys):
    ...
    assert data["rt_avg_ms"] is None and data["rt_p95_ms"] is None and data["rt_p99_ms"] is None
...
def test_stats_naive_iso_treated_as_utc(sample_log, capsys):
    assert main(["stats", "--path", str(sample_log), "--since", "2000-10-10T13:55:39", "--json"]) == 0  # без TZ
    data = json.loads(capsys.readouterr().out)
    assert data["total"] == 2
...
def test_stats_big_log_sanity(tmp_path, capsys):
    ...
    paths = ["/", "/api", "/api/items", "/login", "/search", "/health", "/static/app.js"]
    statuses = [200, 200, 200, 201, 301, 302, 404, 500]
    lines = []
    for i in range(2000):
        ts = (base + __import__("datetime").timedelta(seconds=i)).strftime("%d/%b/%Y:%H:%M:%S +0000")
        path = random.choice(paths)
        st = random.choice(statuses)
        rt = f"{random.random():.3f}"
        lines.append(f'10.0.0.{i % 255} - - [{ts}] "GET {path} HTTP/1.1" {st} {random.randint(0, 999)} "-" "UA" {rt}')
    p = tmp_path / "big.log"
    p.write_text("\n".join(lines) + "\n")
